...
if role == "Agency":
    ...
    st.session_state["selected_agency"] = agy_df.loc[agy_df["agency_name"]==agy_name, "agency_id"].iloc[0]
else:
    st.session_state["selected_agency"] = None
...
with tab1:
    ...
    # Join for display fields (but mask name for Agency)
    ...
    if kw:
        kw_lower = kw.lower()
        view = view[view["role"].str.lower().str.contains(kw_lower) | view["requirements"].str.lower().str.contains(kw_lower)]
    ...
    for _, row in view.iterrows():
        with st.container(border=True):
            ...
            c3.write(f"要件: {row['requirements'][:120]}{'...' if len(row['requirements'])>120 else ''}")
            if role == "Agency":
                if st.button(f"この案件に接続申請する ▶︎", key=f"connect_{row['opportunity_id']}"):
                    # log connection
                    con_df = pd.read_csv(CON_CSV)
                    new = pd.DataFrame([{
                        "connection_id": f"CN_{int(pd.Timestamp.utcnow().timestamp())}_{row['opportunity_id']}",
                        "timestamp": datetime.utcnow().isoformat(),
                        "agency_id": st.session_state["selected_agency"],
